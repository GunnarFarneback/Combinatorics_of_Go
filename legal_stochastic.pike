...
mapping(int:float) count_legal_boards(int height, int width)
{
  ...
  float base_legal_probability = 1.0;
  ...
  for (int j = 0; j < width; j++) {
    for (int i = 0; i < height; i++) {
      write("%d %d\n", j, i);
      int state_sum = Array.sum(values(state_count));
      array(int) states = ({});
      foreach (state_count; int state; int count)
	  states += ({state}) * count;
      state_count = ([]);
      for (int k = 0; k < 10000; k++)
	  state_count[states[random(state_sum)]]++;
	...
	write("%s %d\n", state_to_string(state, height), state_count[state]);
      ...
      state_sum = Array.sum(values(state_count));
      base_legal_probability *= state_sum / 30000.0;
    }
    ...
    results[j+1] = base_legal_probability * ((float) sum) / Array.sum(values(state_count));
  }
...
int main(int argc, array(string) argv)
{
  ...
  mapping(int:float) num_legal = count_legal_boards(height, width);
