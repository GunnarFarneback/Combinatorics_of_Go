...
array(int) sum_state_counts(array(int) a, array(int) b)
{
  if (!a)
    a = ({});
  array(int) result = a[*] + b[*];
  result += a[sizeof(result)..] + b[sizeof(result)..];
  return result;
}
...
array(int) count_legal_boards(int height, int width)
{
  ...
  // that border state.
  mapping(string:array(int)) new_state_count = ([]);
  mapping(string:array(int)) old_state_count = ([]);
  ...
  for (int j = 0; j < width; j++)
	...
	foreach (({"empty", "black", "white"}), string new_stone) {
	  ...
	  if (!bad_state) {
	    ...
	    if (new_stone == "empty")
	      new_state_count[new_state * ""] = sum_state_counts(new_state_count[new_state * ""], ({0}) + old_state_count[state]);
	    else
	      new_state_count[new_state * ""] = sum_state_counts(new_state_count[new_state * ""], old_state_count[state]);
	  }
	}
  ...
  foreach (indices(new_state_count), string state) {
    if (sizeof(state - "." - "X" - "O") == 0)
      sum = sum_state_counts(sum, new_state_count[state]);
  }
...
int main(int argc, array(string) argv)
{
  ...
  array(int) num_legal_per_empty = count_legal_boards(height, width);

  int num_legal = 0;
  int num_empty_in_legal = 0;
  for (int k = 0; k < sizeof(num_legal_per_empty); k++) {
    num_legal += num_legal_per_empty[k];
    num_empty_in_legal += k * num_legal_per_empty[k];
  }
  ...
  write("%dx%d: %d empty in %d legal (average %4.4f empty per legal)\n",
	height, width, num_empty_in_legal, num_legal,
	0.0001 * (10000 * num_empty_in_legal / num_legal/height/width));
  for (int k = 0; k < sizeof(num_legal_per_empty); k++)
    write("%d: %d\n", k, num_legal_per_empty[k]);
